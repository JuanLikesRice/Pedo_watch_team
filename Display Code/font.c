...
void drawPixel(uint8_t xStartIndex, uint8_t yStartIndex, uint8_t brightness) {
    //Still working on figuring out how to draw individual pixel in screen
}
...
void drawCharacter(char character, uint8_t xStartIndex, uint8_t yStartIndex, uint8_t brightness) {
    ...
    const uint8_t* characterFont = font[CHAR_WIDTH * character];
    ...
    for (y_index = 0; y_index < CHAR_WIDTH; y_index++) {
        for (x_index = 0; x_index < CHAR_HEIGHT; x_index++) {
            if ((1 << x_index) & characterFont[y_index]) {
                drawPixel(xStartIndex + x_index, yStartIndex + y_index, brightness);
            }
        }
    }
}
